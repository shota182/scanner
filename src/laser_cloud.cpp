...
public:
    LaserPointCloudCalculator() : nh_("~"){
        ...
        points2d_pub_ = nh_.advertise<sensor_msgs::PointCloud>("/laser/points2d", 1);
        points3d_pub_ = nh_.advertise<sensor_msgs::PointCloud>("/laser/points3d", 1);
    }
    ...
    void imageCallback(const sensor_msgs::ImageConstPtr& binary_image_msg) {
        try {
            ...
            points3d_msg.header.frame_id = "/camera"; // フレーム違うので修正が必要
            std::vector<cv::Point> points;
            cv::findNonZero(cv_ptr->image, points); // get 255 points

            // Eigen::MatrixXd型の方向ベクトルを取得
            Eigen::MatrixXd pos_image(3, points.size()), pos_vector(3, points.size());
            for (size_t i = 0; i < points.size(); ++i) {
                geometry_msgs::Point32 point;
                point.x = points[i].x;
                point.y = points[i].y;
                point.z = 0.;
                points2d_msg.points.push_back(point);
                cv::Point3d point3d = camera_model_.projectPixelTo3dRay(cv::Point2d(points[i].x, points[i].y));
